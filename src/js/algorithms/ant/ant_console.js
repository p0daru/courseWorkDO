...
export function ant_console(n, t_matrix, alpha) {
    ...
    for (let k = 1; k <= lMax; k++) {
        ...
        while (result_it.length < n) {
            for (let x = 0; x < n; x++)
                iteration_matrix[x][student] = Infinity;
            ...
            for (let i = 0; i < n; i++) {
                if (iteration_matrix[student][i] != Infinity) {
                    formula_denominator += parseFloat((tay_matrix[student][i]) ** alpha) * ((1 / t_matrix[student][i]) ** beta);
                }
            }
            for (let i = 0; i < n; i++) {
                let it = 0;
                if (iteration_matrix[student][i] != Infinity) {
                    it = parseFloat(((tay_matrix[student][i]) ** alpha) * ((1 / t_matrix[student][i]) ** beta) / formula_denominator).toFixed(4);
                    if (res_set.size === 0)
            ...
            console.log("Поточний розклад: ", result_it);
            console.log();
        }
        // Обчислення ЦФ :
        ...
        console.log("Результат ітерації", k, ": ", result_it);
            ...
            for (let j = 0; j < n; j++) {
                if (t_matrix[i][j] != Infinity) {
                    if (visited_array[i][j] === 1)
                        tay_matrix[i][j] = parseFloat((1 - p) * tay_matrix[i][j] + delta_tay).toFixed(4);
                    else
